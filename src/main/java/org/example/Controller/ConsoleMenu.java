...
public class ConsoleMenu {
    ...
    public static void start() {
        while (true) {
            ...
            System.out.println("1. Intra in contul tau");
            System.out.println("2. Nu ai cont? Inregistreaza-te");
            System.out.print("Optiune: ");
    ...
    private static void login(UserRole rol) {
        ...
        if (user.getRole() != rol) {
            System.out.println("Din pacate accesul a fost respins.");
            return;
        }

        System.out.println("Logare realizata cu succes! Bine ai venit, " + user.getName());
        if (rol == UserRole.DOCTOR) {
            doctorMenu((Doctor) user);
        } else {
            patientMenu((Pacient) user);
        }
    }
        ...
        while (true) {
            ...
            switch (opt) {
                ...
                case 3 -> {
                    ...
                    Pacient pacient = patientService.findById((long) patientId);
                    if (pacient == null) {
                    ...
                    medicalRecord.addMedicalRecord(pacient, doctor, diagnostic, tratament);
                    System.out.println("Diagnosticul si tratamentul au fost adaugate cu succes!");
                }
    ...
    private static void patientMenu(Pacient patient) {
        while (true) {
            System.out.println("\n--- Meniu Pacient ---");
            System.out.println("1. Solicita o programare");
            ...
            switch (opt) {
                ...
                case 2 -> {
                    // TODO: afisare istoricul programarilor
    ...
    private static void showMedicalHistory(Pacient patient) {
        List<IstoricMedical> records = medicalRecord.getRecordsForPatient(patient);

        if (records.isEmpty()) {
            System.out.println(" Nu ai inregistrari.");
            return;
        }

        System.out.println("\n< Istoric >");
        for (IstoricMedical r : records) {
